  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const km = haversineKm(
        ...
        cleanup.report.latitude,
        cleanup.report.longitude,
      );
      setDistKm(km);
    })();
  }, [cleanup]);
  ...
  const handleStartTask = async () => {
    ...
    if (distKm === null) {
      showModal({
        ...
        message:
          "Allow location access so we can confirm you are at the cleanup site.",
      });
      return;
    }
    if (Number(distKm) > 0.2) {
      showModal({
        variant: "info",
        title: "You are too far away",
        message:
          "Move within 200 metres of the report location before starting this task.",
      });
      return;
    }
    setStarting(true);
    try {
      ...
      showModal({
        ...
        message: e.message ?? "Could not start task.",
      });
    } finally {
      setStarting(false);
    }
  ...
  const handleAcceptTask = async () => {
    ...
    try {
      ...
      showModal({
        ...
        title: "Task accepted",
        message:
          "Open navigation when you are ready to travel to the location.",
      });
    } catch (e: any) {
      showModal({
        variant: "error",
        ...
        <View style={styles.ctaContainer}>
            ...
            <View style={styles.assignmentActions}>
              ...
              >
                {responding ? (
                  ...
                  <Text style={styles.startBtnText}>Accept task</Text>
                )}
              </Pressable>
            </View>
          )}
          ...
          {cleanup.status === "COMPLETED" && (
            <Pressable
              ...
              onPress={() =>
                router.push(`/(worker)/task/${cleanup.id}/completed` as any)
              }
            >
    ...
    >
      ...
      <View style={styles.infoCard}>
        {[
          ...
          ...(distKm
            ? [{ icon: "📏", label: "Distance", value: `${distKm} km away` }]
            : []),
          {
            icon: "🗑️",
            label: "Waste Type",
            value: wasteCategoryLabel[report.wasteCategory ?? ""] ?? "Unknown",
          },
          {
            icon: "🚮",
            label: "Dump Type",
            value: dumpTypeLabel[report.dumpType ?? ""] ?? "N/A",
          },
          {
            icon: "⚡",
            label: "Attention",
            value: report.attention === "URGENT" ? "🔴 URGENT" : "🟡 Normal",
          },
          {
            icon: "📊",
            label: "Severity Score",
            value: report.severityScore
              ? `${report.severityScore.toFixed(0)}/100`
              : "N/A",
          },
          {
            icon: "👤",
            label: "Assigned By",
            value: cleanup.assignedByRef.name,
          },
          {
            icon: "🕐",
            label: "Assigned At",
            value: formatDateTime(cleanup.assignedAt),
          },
          ...(report.workersNeeded
            ? [
                {
                  icon: "👷",
                  label: "Workers Needed",
                  value: String(report.workersNeeded),
                },
              ]
            : []),
          ...(report.truckSize
            ? [{ icon: "🚛", label: "Truck Size", value: report.truckSize }]
            : []),
        ].map((row) => (
